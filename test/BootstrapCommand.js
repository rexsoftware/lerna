...
describe("BootstrapCommand", () => {
  ...
  describe("peer dependencies in packages in the repo", () => {
    ...
    it("should bootstrap peer dependencies", (done) => {
      const bootstrapCommand = new BootstrapCommand([], {});
      ...
      bootstrapCommand.runCommand(exitWithCode(0, (err) => {
        ...
        try {
          assert.ok(!pathExists.sync(path.join(testDir, "lerna-debug.log")), "lerna-debug.log should not exist");
          assert.ok(pathExists.sync(path.join(testDir, "packages/package-1/node_modules/package-2")), "The linkable peer dependency should be installed");
          assert.ok(installed, "The external peer dependency should be installed");
          done();
        } catch (err) {
          done(err);
        }
